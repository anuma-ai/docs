...
Common server tools include web search and image generation. Check out the [full list of server tools](/tools/list) for all available options.

## Usage

Specify which tools to enable by name when sending a message through [`useChat`](/sdk/react/Hooks/useChat) or [`useChatStorage`](/sdk/react/Hooks/useChatStorage):
